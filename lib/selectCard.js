...
const {
  ...
  eligibleClothingItems, isStripStreakBlocked,
} = require('./gameHelpers');
...
function selectCard(allCards, type, performer, targets, chosenLevel, usedCardIds, options = {}) {
  ...
  function targetPasses(card, cand) {
    ...
    // {{ct}} cards strip the TARGET — gate on the target's wardrobe + their streak.
    if (isClothingRemovalCard(card) && clothingTokenKind(card) === 'ct') {
      if (isStripStreakBlocked(cand)) return false;
      if (eligibleClothingItems(card, cand.clothingItems).length === 0) return false;
    }
    return true;
  }
  ...
  function passes(card, level) {
    ...
    if (isClothingRemovalCard(card) && clothingTokenKind(card) !== 'ct') {
      ...
      if (eligibleClothingItems(card, performer.clothingItems).length === 0) return false;
    }
