 ...
 * @param {string}   [options.rouletteMode] - 'off'|'exact'|'plus5'|'block'
 * @returns {{ card: object|null, recycled: boolean }}
 */
function selectCard(allCards, type, performer, target, chosenLevel, usedCardIds, options = {}) {
  ...
  function passes(card, level) {
    ...
    // Step 1: level filter
    if (rouletteMode === 'off' || rouletteMode === 'exact') {
      if (card.level !== level) return false;
    } else if (rouletteMode === 'block') {
      if (card.level > level) return false;
    }
    // 'plus5' allows level ± a range; simple implementation: level or level+1
    // (full roulette range is Phase 2 — for now treat as exact)
    ...
    // Step 5: orientation filter (only when card explicitly requires it)
    if (card.orientationMatters && target) {
      if (!orientationCompatible(performer, target)) return false;
    }

    // Step 6: body type filter
    if (card.bodyTypeRelevant) {
      if (card.bodyTypeRequired && performer.bodyType !== card.bodyTypeRequired) return false;
    }
  ...
  for (let lvl = chosenLevel; lvl >= 1; lvl--) {
    ...
    if (pool.length > 0) {
      return { card: pickWeighted(pool, performer), recycled: false };
    }
  }
  ...
  for (let lvl = chosenLevel; lvl >= 1; lvl--) {
    ...
    if (pool.length > 0) {
      return { card: pickWeighted(pool, performer), recycled: true };
    }
  }
  ...
  if (type === 'dare') {
    ...
    if (safeTruth) return { card: safeTruth, recycled: false };
  }

  return { card: null, recycled: false };
}
 ...
 * orientation-relevant dare (Phase 5 detail — minimal impl for now).
 */
