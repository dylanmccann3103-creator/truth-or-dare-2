...
function selectCard(allCards, type, performer, targets, chosenLevel, usedCardIds, options = {}) {
  ...
  // passes() enforces card.type === type so cannot be reused here — check safety inline.
  if (type === 'dare') {
    const safeTruth = allCards.find(c =>
      c.type === 'truth' && c.level === 1 &&
      !(c.tags && c.tags.some(t => (performer.limits || []).includes(t))) &&
      targetGateOk(c)
    );
